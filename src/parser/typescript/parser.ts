...
export const parse: Parse<Options> = async (entrypoint, options) => {
	...
	return parseAST(program, entrypoint, log)
}
...
export function parseAST(
	...
	if (!sourceFile) {
		log.fail(
			`Could not find entrypoint: ${entrypoint}. Make sure your project contains \`tsconfig.json\`!`
		)
		return
	}
	...
	if (errors.length > 0) {
		log.error(`Found \`${errors.length}\` error(s). Fix and re-run!`)
		return
	}
	...
	function tryGetOrigin(exportName: string) {
		...
		ts.forEachChild(sourceFile as ts.SourceFile, (node) => {
